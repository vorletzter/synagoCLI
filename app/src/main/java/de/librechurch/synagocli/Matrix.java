...
public class Matrix {
    ...
    private ArrayList mUserSessionsLoaded = new ArrayList<>();
    private ArrayList mUserSessionsLoading = new ArrayList<>();
    ...
    public void addSession(MXSession mySession) {
        this.mUserSessionsLoaded.add(mySession.getMyUserId());
        this.mMXSessions.add(mySession);
    }

    public ArrayList getAllActiveUserIds() {
        return mUserSessionsLoaded;
    }
    ...
    public synchronized MXSession activateSession(HomeServerConnectionConfig hsConfig) {
        ...
        if (mUserSessionsLoading.contains(hsConfig.getCredentials().userId) == false) {
            mUserSessionsLoading.add(hsConfig.getCredentials().userId);
            ...
            Log.d(LOG_TAG, "Session is already loaded/loading. Ignoring request");
        }
        return null;
    }
}
